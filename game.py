            ...
            if orientation == 0:
                ...
                if all(grid[row][col + i] == 0 for i in range(length)):
                    for i in range(length):
                        # Set player's battleship value to 4
                        grid[row][col + i] = 4
                    placed = True
            else:
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if (
                ...
                and clicked_row < len(AI_grid)
                and clicked_col < len(AI_grid[clicked_row])
            ):
                turn_label = font.render("PLAYER 1 TURN", True, WHITE)
                # clicked_col -= ATTACK_SIZE
                result = check_hit(clicked_row, clicked_col, AI_grid)
                print("Player's Attack Coordinates: ",
                      clicked_row, clicked_col, result)
    ...
    if game_over:
        ...
        window.blit(game_over_label, (WINDOW_WIDTH //
                    2 - 120, WINDOW_HEIGHT // 2 - 20))
